...
export const brands = pgTable(
  ...
  (table) => [
    index("idx_brands_organization").using(
      "btree",
      table.organizationId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_brands_parent").using(
      "btree",
      table.parentBrandId.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
...
export const organizations = pgTable(
  ...
  (table) => [unique("organizations_code_key").on(table.code)],
);
...
export const brandRules = pgTable(
  ...
  (table) => [
    index("idx_brand_rules_type").using(
      "btree",
      table.ruleType.asc().nullsLast().op("text_ops"),
    ),
    foreignKey({
    ...
    unique("brand_rules_brand_id_rule_type_rule_key_key").on(
      table.brandId,
      table.ruleType,
      table.ruleKey,
    ),
...
export const companies = pgTable(
  "companies",
  {
    ...
    description: text(),
    estimatedMarketcap: numeric("estimated_marketcap", {
      precision: 20,
      scale: 2,
    }),
    createdAt: timestamp("created_at", { withTimezone: true, mode: "string" })
  ...
  (table) => [
    index("idx_companies_name_trgm").using(
      "gin",
      table.companyName.asc().nullsLast().op("gin_trgm_ops"),
    ),
    index("idx_companies_symbol_trgm").using(
      "gin",
      table.companySymbol.asc().nullsLast().op("gin_trgm_ops"),
    ),
    foreignKey({
...
export const userSessions = pgTable(
  ...
  (table) => [
    index("idx_user_sessions_user_expires").using(
      "btree",
...
export const crossBrandRules = pgTable(
  "cross_brand_rules",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
  ...
  (table) => [
    index("idx_cross_brand_rules_trigger").using(
      "btree",
      table.triggerEvent.asc().nullsLast().op("int4_ops"),
...
export const userRoleAssignments = pgTable(
  ...
  (table) => [
    index("idx_user_role_assignments_user_brand").using(
      "btree",
    ...
    unique("user_role_assignments_user_id_role_id_brand_id_key").on(
      table.userId,
      table.roleId,
      table.brandId,
    ),
...
export const mightyCallTokens = pgTable(
  "mighty_call_tokens",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    userId: uuid("user_id").notNull(),
  ...
  (table) => [
    ...
    unique("mighty_call_tokens_user_id_brand_id_key").on(
      table.userId,
      table.brandId,
    ),
  ],
...
export const additionalContacts = pgTable(
  "additional_contacts",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
  ...
  (table) => [
    index("idx_additional_contacts_company").using(
      "btree",
...
export const leads = pgTable(
  ...
  (table) => [
    index("idx_leads_company").using(
      "btree",
      table.companyId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_leads_full_name_trgm").using(
      "gin",
...
export const automationRuns = pgTable(
  "automation_runs",
  {
    ...
    recordsProcessedCount: integer("records_processed_count")
      .default(0)
      .notNull(),
    recordsSucceededCount: integer("records_succeeded_count")
      .default(0)
      .notNull(),
  ...
  (table) => [
    index("idx_automation_runs_name_started").using(
      "btree",
      table.automationName.asc().nullsLast().op("text_ops"),
      table.startedAt.asc().nullsLast().op("text_ops"),
    ),
    index("idx_automation_runs_status_started").using(
      "btree",
      table.status.asc().nullsLast().op("text_ops"),
      table.startedAt.asc().nullsLast().op("text_ops"),
    ),
    index("idx_automation_runs_target_started").using(
      "btree",
      table.targetTable.asc().nullsLast().op("text_ops"),
      table.startedAt.asc().nullsLast().op("text_ops"),
    ),
    foreignKey({
...
export const leadBrandState = pgTable(
  "lead_brand_state",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    ...
    autoDialMightyCall: boolean("auto_dial_mighty_call")
      .default(false)
      .notNull(),
    lastModifiedTimeLeadType: timestamp("last_modified_time_lead_type", {
      withTimezone: true,
      mode: "string",
    }),
    lastUpdatedToBeCalledAt: timestamp("last_updated_to_be_called_at", {
      withTimezone: true,
      mode: "string",
    }),
    updatedAt: timestamp("updated_at", { withTimezone: true, mode: "string" }),
  ...
  (table) => [
    index("idx_lbs_brand_lead_type").using(
      "btree",
      table.brandId.asc().nullsLast().op("uuid_ops"),
      table.leadType.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_lbs_brand_to_be_called").using(
      "btree",
      table.brandId.asc().nullsLast().op("uuid_ops"),
      table.toBeCalledByUserId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_lbs_followup")
      .using(
        "btree",
        table.brandId.asc().nullsLast().op("date_ops"),
        table.followUpDate.asc().nullsLast().op("uuid_ops"),
      )
      .where(sql`(follow_up_date IS NOT NULL)`),
    index("idx_lbs_hot_user")
      .using(
        "btree",
        table.brandId.asc().nullsLast().op("uuid_ops"),
        table.toBeCalledByUserId.asc().nullsLast().op("uuid_ops"),
      )
      .where(sql`((lead_type)::text = 'hot'::text)`),
    foreignKey({
      columns: [table.brandId],
      foreignColumns: [brands.id],
    ...
    unique("lead_brand_state_lead_id_brand_id_key").on(
      table.leadId,
      table.brandId,
    ),
  ],
...
export const callLogs = pgTable(
  ...
  (table) => [
    index("idx_call_logs_brand_called").using(
      "btree",
      table.brandId.asc().nullsLast().op("timestamptz_ops"),
      table.calledAt.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_call_logs_lead_brand_called").using(
      "btree",
      table.leadId.asc().nullsLast().op("uuid_ops"),
      table.brandId.asc().nullsLast().op("timestamptz_ops"),
      table.calledAt.asc().nullsLast().op("timestamptz_ops"),
    ),
    index("idx_call_logs_user_called").using(
      "btree",
      table.userId.asc().nullsLast().op("uuid_ops"),
      table.calledAt.asc().nullsLast().op("uuid_ops"),
    ),
...
export const emailLogs = pgTable(
  ...
  (table) => [
    index("idx_email_logs_lead_brand_sent").using(
      "btree",
...
export const smsLogs = pgTable(
  ...
  (table) => [
    index("idx_sms_logs_lead_brand_sent").using(
      "btree",
      table.leadId.asc().nullsLast().op("uuid_ops"),
      table.brandId.asc().nullsLast().op("timestamptz_ops"),
      table.sentAt.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
      columns: [table.brandId],
      foreignColumns: [brands.id],
...
export const hotLeadEvents = pgTable(
  "hot_lead_events",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    leadId: uuid("lead_id").notNull(),
  ...
  (table) => [
    index("idx_hot_lead_events_brand_event").using(
      "btree",
      table.brandId.asc().nullsLast().op("timestamptz_ops"),
      table.eventAt.asc().nullsLast().op("timestamptz_ops"),
    ),
    foreignKey({
      columns: [table.brandId],
      foreignColumns: [brands.id],
...
export const leadFlags = pgTable(
  ...
  (table) => [
    index("idx_lead_flags_brand_open")
      .using(
        "btree",
        table.brandId.asc().nullsLast().op("text_ops"),
        table.flagType.asc().nullsLast().op("uuid_ops"),
        table.createdAt.asc().nullsLast().op("uuid_ops"),
      )
      .where(sql`(resolved_at IS NULL)`),
    index("idx_lead_flags_lead_brand_type").using(
      "btree",
      table.leadId.asc().nullsLast().op("text_ops"),
      table.brandId.asc().nullsLast().op("uuid_ops"),
...
export const level2Requests = pgTable(
  "level_2_requests",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    leadId: uuid("lead_id").notNull(),
  ...
  (table) => [
    index("idx_level_2_brand_status").using(
      "btree",
      table.brandId.asc().nullsLast().op("text_ops"),
...
export const userDailyScores = pgTable(
  "user_daily_scores",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    userId: uuid("user_id").notNull(),
  ...
  (table) => [
    index("idx_user_daily_scores_board").using(
      "btree",
      table.brandId.asc().nullsLast().op("int4_ops"),
      table.scoreDate.asc().nullsLast().op("uuid_ops"),
      table.points.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
      columns: [table.brandId],
      foreignColumns: [brands.id],
    ...
    unique("user_daily_scores_user_id_brand_id_score_date_key").on(
      table.userId,
      table.brandId,
      table.scoreDate,
    ),
...
export const userMonthlyScores = pgTable(
  "user_monthly_scores",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    userId: uuid("user_id").notNull(),
  ...
  (table) => [
    index("idx_user_monthly_scores_board").using(
      "btree",
      table.brandId.asc().nullsLast().op("int4_ops"),
      table.yearMonth.asc().nullsLast().op("uuid_ops"),
      table.points.asc().nullsLast().op("uuid_ops"),
    ),
    foreignKey({
      columns: [table.brandId],
    ...
    unique("user_monthly_scores_user_id_brand_id_year_month_key").on(
      table.userId,
      table.brandId,
      table.yearMonth,
    ),
...
export const auditLog = pgTable(
  ...
  (table) => [
    index("idx_audit_log_automation_run").using(
      "btree",
      table.changedByAutomationRunId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_audit_log_change_group").using(
      "btree",
      table.changeGroupId.asc().nullsLast().op("uuid_ops"),
    ),
    index("idx_audit_log_table_record_time").using(
      "btree",
      table.tableName.asc().nullsLast().op("uuid_ops"),
      table.recordId.asc().nullsLast().op("uuid_ops"),
      table.changedAt.asc().nullsLast().op("text_ops"),
    ),
    foreignKey({
    ...
    check(
      "audit_log_check",
      sql`(changed_by_user_id IS NULL) OR (changed_by_automation_run_id IS NULL)`,
    ),
    check(
      "audit_log_operation_check",
      sql`operation IN ('INSERT','UPDATE','DELETE')`,
    ),
    check(
      "audit_log_actor_type_check",
      sql`actor_type IN ('user','automation','api','db_direct','migration','system')`,
    ),
  ],
);

export const brandLaunchEvents = pgTable(
  "brand_launch_events",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    brandId: uuid("brand_id").notNull(),
  ...
  (table) => [
    index("idx_brand_launch_events_brand_time").using(
      "btree",
      table.brandId.asc().nullsLast().op("timestamptz_ops"),
...
export const userBrands = pgTable(
  ...
  (table) => [
    ...
    primaryKey({
      columns: [table.userId, table.brandId],
      name: "user_brands_pkey",
    }),
  ],
);

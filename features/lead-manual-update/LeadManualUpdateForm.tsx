...
function getLeadLabel(row: LeadDirectoryRow) {
  ...
  const leadId = row.lead_id || row.lead || row.email || "No ID";
...
export function LeadManualUpdateForm() {
  ...
  const leadRows = useMemo<LeadDirectoryRow[]>(
    () => getStoredLeads(),
    [],
  );
  ...
  const leadOptions = useMemo(
    () =>
      leadRows.map((row, index) => ({
        ...
        value: row.lead_id || row.lead || row.email || String(index),
      })),
    [leadRows],
  );
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    showSuccessToast("Lead manual update submitted locally. This is a dummy action.");
  };
        ...
        <CardContent className="p-0">
          <div className="border-b border-slate-200 px-5 py-4 dark:border-slate-700 sm:px-6">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Create a manual update entry for a lead using the existing agent workflow fields.
            </p>
          </div>

          <form onSubmit={handleSubmit} className="grid gap-5 px-5 py-5 sm:px-6 sm:py-6">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              {isAdmin ? (
                <Select
                  ...
                  onChange={(value) => updateField("campaignType", String(value))}
                  className={selectClassName}
                />
              ) : (
                <TextInput
